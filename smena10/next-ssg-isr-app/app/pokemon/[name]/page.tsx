...
async function getPokemon(name: string): Promise<Pokemon> {
  ...
  if (!res.ok) notFound(); // 👈 activa el not-found.tsx
  return res.json();
}
...
const typeColors: Record<string, string> = {
  ...
  normal: 'bg-gray-400', fighting: 'bg-orange-700', flying: 'bg-indigo-400',
    ...
    <div className="p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className={`bg-gradient-to-r from-black ${typeColors[pokemon.types[0].type.name]} p-8`}>
          ...
          <p className="text-white text-center text-xl mt-2">#{pokemon.id.toString().padStart(3, '0')}</p>
        </div>
        <div className="p-8">
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  {pokemon.types.map((type) => (
                    <span key={type.type.name} className={`${typeColors[type.type.name] || 'bg-gray-400'} text-white px-4 py-2 rounded-full font-semibold capitalize`}>
                      {type.type.name}
                    </span>
                  ))}
                </div>
                ...
                {pokemon.stats.map((stat) => (
                  <div key={stat.stat.name} className="mb-3 text-gray-700">
                    <div className="flex justify-between mb-1">
                      <span className="capitalize font-semibold">{stat.stat.name.replace('-', ' ')}</span>
        ...
        <div className="p-8 bg-gray-50">
          <Link href="/pokemon" className="inline-block bg-black hover:bg-purple-900 text-white font-bold py-3 px-6 rounded-lg transition">
            ← Volver al Pokédex
          </Link>
        </div>
      </div>
